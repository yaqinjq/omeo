...
class PositionController extends Controller
{
    ...
    public function storeDepartment(Request $request): RedirectResponse|\Illuminate\Http\JsonResponse
    {
    ...
    public function orgChart(): \Illuminate\View\View
    {
        ...
        // ── Tab "Builder Struktur" — 1 tree gabungan Department/Brand/Outlet/Karyawan ────
        ...
        $nodeChildrenByParent = $allNodes->filter(fn ($n) => $n->parent_id)->groupBy('parent_id');
        $nodeRoots            = $allNodes->whereNull('parent_id')->values();

        // Dropdown "Pilih Karyawan" — semua karyawan aktif (bukan cuma yang
        // belum dipakai node manapun, supaya karyawan yang sedang mengisi
        // node yang diedit tetap muncul di listnya); duplikasi tetap ditolak
        // server-side di OrgChartNodeController.
        $allEmployeesForPicker = Employee::whereNotIn('status_employment', ['resigned', 'terminated'])
            ->orderBy('full_name')
            ->get(['id', 'full_name']);

        $allDepartmentsForPicker = Department::orderBy('name')->get(['id', 'name']);
        $allOutletsForPicker     = Outlet::orderBy('name')->get(['id', 'name', 'brand_name']);
        $allPositionsForPicker   = Position::orderBy('name')->get(['id', 'name']);
        $existingBrandNames      = Outlet::whereNotNull('brand_name')->where('brand_name', '!=', '')
        ...
        return view('positions.org_chart', compact(
            ...
            'nodeRoots', 'nodeChildrenByParent', 'allEmployeesForPicker',
            'allDepartmentsForPicker', 'allOutletsForPicker', 'allPositionsForPicker', 'existingBrandNames'
        ));
    }
