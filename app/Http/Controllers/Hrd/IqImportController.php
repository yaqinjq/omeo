...
class IqImportController extends Controller
{
    ...
    {
        $iqForms = Schema::hasTable('forms')
            ? AssessmentForm::query()
                ->where('type', AssessmentForm::TYPE_IQ)
                ->orderByDesc('id')
                ->get(['id', 'name', 'duration_minutes', 'is_active'])
            : collect();

        $templateHeaders = $this->importService->templateHeaders();
        $moduleWarning = Schema::hasTable('forms') ? null : 'Schema Form Dinamis belum lengkap di environment ini. Import IQ dibuka dalam mode aman.';

        return view('hrd.forms.import_iq', compact('iqForms', 'templateHeaders', 'moduleWarning'));
    }
    ...
    public function downloadCsv()
    {
        ...
        return Response::streamDownload($callback, 'template-import-soal-iq.csv', [
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'form_id' => [
                ...
                'nullable',
                'integer',
                Rule::exists('forms', 'id')->where(fn ($query) => $query->where('type', AssessmentForm::TYPE_IQ)),
            ],
        ], [
            ...
            'form_name.required_if' => 'Nama form IQ wajib diisi untuk opsi Buat Form Baru.',
            ...
            'form_id.required_if' => 'Pilih form IQ tujuan untuk opsi Append.',
            'form_id.exists' => 'Form IQ tujuan tidak ditemukan.',
        ]);
        ...
        $flashMessage = $result['imported_questions'] > 0
            ? 'Import bank soal IQ selesai diproses.'
            : 'Tidak ada baris valid yang berhasil diimport.';

        return redirect()
            ->route('hrd.import.iq.index')
