...
<div class="bg-white border rounded-lg p-4 space-y-6">
  <div>
    <h1 class="text-lg font-semibold">Import Soal IQ</h1>
    <p class="text-sm text-slate-600">Import bank soal IQ dari file CSV/XLSX ke Form Dinamis recruitment.</p>
  </div>
  ...
  @if(session('import_summary'))
    ...
    <div class="rounded border border-blue-200 bg-blue-50 p-4 space-y-2">
      ...
      @if(!empty($summary['form_id']))
        <a href="{{ route('hrd.forms.edit', $summary['form_id']) }}" class="inline-block text-sm underline text-blue-800">Buka Form Builder: {{ $summary['form_name'] ?? 'Form IQ' }}</a>
      @endif
  ...
  <section class="space-y-3">
    ...
    <div class="rounded border bg-slate-50 p-3 text-sm text-slate-700 space-y-2">
      ...
      <ul class="list-disc ml-5 space-y-1 text-xs">
        ...
        <li><b>correct_index</b> (1..5) dipakai untuk kunci single choice (radio/dropdown).</li>
  ...
  <section class="space-y-3">
    ...
    <div class="flex flex-wrap gap-2">
      <a href="{{ route('hrd.import.iq.template.csv') }}" class="px-3 py-2 rounded border text-sm hover:bg-slate-50">Download Template CSV</a>
      <a href="{{ route('hrd.import.iq.template.xlsx') }}" class="px-3 py-2 rounded bg-slate-900 text-white text-sm">Download Template XLSX</a>
    </div>
  </section>

  <section class="space-y-3">
    <h2 class="text-sm font-semibold">C. Upload & Import</h2>
    <form method="POST" action="{{ route('hrd.import.iq.store') }}" enctype="multipart/form-data" class="space-y-4 border rounded p-4">
      @csrf
      ...
      <div class="space-y-2">
        ...
        <label class="flex items-start gap-2 text-sm">
          ...
          <span>Create new form IQ</span>
        </label>
        <label class="flex items-start gap-2 text-sm">
          <input type="radio" name="target_mode" value="append" {{ old('target_mode') === 'append' ? 'checked' : '' }}>
          <span>Append ke form IQ existing</span>
        </label>
      </div>

      <div id="createFields" class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <label class="block text-sm font-medium mb-1">Nama Form IQ Baru</label>
          <input type="text" name="form_name" value="{{ old('form_name') }}" class="block w-full text-sm border rounded px-3 py-2" placeholder="Contoh: IQ Batch Maret 2026">
        </div>
        <div>
      ...
      <div id="appendFields" class="hidden">
        <label class="block text-sm font-medium mb-1">Pilih Form IQ Existing</label>
        <select name="form_id" class="block w-full text-sm border rounded px-3 py-2">
          <option value="">-- Pilih Form IQ --</option>
          @foreach($iqForms as $form)
