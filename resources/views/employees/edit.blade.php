...
<div class="bg-white border rounded-lg p-4">
  <div class="text-lg font-semibold mb-3">Edit Karyawan</div>
  ...
  <form method="POST" action="{{ route('employees.update', $employee) }}">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div>
        ...
        <input type="number" min="0" step="0.01" name="current_salary" value="{{ old('current_salary', $employee->current_salary ?? '') }}" class="w-full border rounded p-2" placeholder="Contoh: 6500000">
      </div>
      <div>
        <label class="text-sm">Tanggal Efektif Gaji</label>
        <input type="date" name="salary_effective_date" value="{{ old('salary_effective_date', isset($employee) && $employee->join_date ? $employee->join_date->format('Y-m-d') : now()->format('Y-m-d')) }}" class="w-full border rounded p-2">
      </div>
      <div class="md:col-span-2">
        <label class="text-sm">Catatan Perubahan Gaji</label>
        <textarea name="salary_notes" rows="3" class="w-full border rounded p-2" placeholder="Contoh: Penyesuaian setelah appraisal / update payroll">{{ old('salary_notes') }}</textarea>
      </div>
    </div>
