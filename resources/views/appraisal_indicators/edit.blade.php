...
<div class="bg-white border rounded-lg p-4 max-w-3xl">
  <div class="text-lg font-semibold mb-3">Edit Indikator</div>
  <form method="POST" action="{{ route('appraisal-indicators.update',$indicator) }}">
    @csrf @method('PUT')
  ...
  <div>
    ...
    <select name="template_id" class="w-full border rounded p-2" required>
      @foreach($templates as $t)
        <option value="{{ $t->id }}" @selected((int) old('template_id', $indicator->template_id) === $t->id)>{{ $t->name }}</option>
      @endforeach
    </select>
  </div>
  <div>
  ...
  <div>
    ...
    <div class="mt-5 rounded-lg border border-slate-200 bg-slate-50 p-3">
      ...
      <div class="space-y-2">
        @php $existingRubric = $indicator->scale_labels ?? []; @endphp
        @for($star = 5; $star >= 1; $star--)
        <div class="flex gap-2 items-start">
          <div class="w-20 flex-shrink-0 pt-2 text-xs font-semibold text-slate-600">Skor {{ $star }}</div>
          <textarea name="rubric[{{ $star }}]" rows="2" class="w-full border rounded p-2 text-sm" placeholder="Contoh: performa sangat tinggi, bekerja mandiri, proaktif...">{{ old("rubric.$star", $existingRubric[$star] ?? '') }}</textarea>
        </div>
        @endfor
      </div>
    </div>
